    ...
    IRequestHandler<GetProductionWipQuery, List<ProductionOrderReadModel>>
{
    ...
    public ProductionQueryHandler(ProductionReadDbContext readDb)
    {
        _readDb = readDb;
    }
    ...
    {
        return await _readDb.ProductionOrders.FindAsync(new object[] { request.Id }, ct);
    }

    public async Task<List<ProductionOrderReadModel>> Handle(SearchProductionOrdersQuery request, CancellationToken ct)
    {
        var query = _readDb.ProductionOrders.AsNoTracking().AsQueryable();
    ...
    {
        var wipStatuses = new[] { "Released", "InProgress", "PartiallyCompleted" };
        var query = _readDb.ProductionOrders.AsNoTracking().Where(x => wipStatuses.Contains(x.Status));
        if (!string.IsNullOrEmpty(request.MaterialId)) query = query.Where(x => x.MaterialId == request.MaterialId);
