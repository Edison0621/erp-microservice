        ...
        IRequestHandler<MoveDepartmentCommand, bool>
{
    public async Task<Guid> Handle(CreatePositionCommand r, CancellationToken ct)
    {
        ...
        Position pos = Position.Create(id, r.Name, r.Description);
        await posRepo.SaveAsync(pos);
        return id;
    }

    public async Task Handle(UpdateUserProfileCommand r, CancellationToken ct)
    {
        User? user = await userRepo.LoadAsync(r.UserId);
        if (user == null) return;
    ...
    {
        Role? role = await roleRepo.LoadAsync(r.RoleId);
        if (role == null) return;
    ...
    {
        Guid id = Guid.NewGuid();
        string? hash = BCrypt.Net.BCrypt.HashPassword(r.Password);
        User user = User.Create(id, r.Username, r.Email, r.DisplayName, hash);
        await userRepo.SaveAsync(user);
        return id;
    }
    ...
    {
        UserReadModel? readModel = await readDb.Users.FirstOrDefaultAsync(u => u.Username == r.Username, ct);
        if (readModel == null || !BCrypt.Net.BCrypt.Verify(r.Password, readModel.PasswordHash))
            throw new Exception("Invalid credentials");

        User? user = await userRepo.LoadAsync(readModel.UserId);
    ...
    {
        Guid id = Guid.NewGuid();
        Role role = Role.Create(id, r.RoleName, r.RoleCode, r.IsSystemRole);
        await roleRepo.SaveAsync(role);
        return id;
    }
    ...
    {
        Guid id = Guid.NewGuid();
        Department dept = Department.Create(id, r.Name, r.ParentId, r.Order);
        await deptRepo.SaveAsync(dept);
        return id;
    }

    public async Task<bool> Handle(MoveDepartmentCommand r, CancellationToken ct)
    {
        Department? dept = await deptRepo.LoadAsync(r.DepartmentId);
