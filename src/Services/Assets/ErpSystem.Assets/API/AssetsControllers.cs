...
[ApiController]
[Route("api/v1/assets")]
public class AssetsController(IEventStore eventStore, AssetsReadDbContext readDb) : ControllerBase
{
    [HttpGet]
    ...
    {
        IQueryable<AssetReadModel> query = readDb.Assets.AsQueryable();
        ...
        List<AssetReadModel> assets = await query.OrderBy(a => a.AssetNumber).ToListAsync();
        return this.Ok(new { items = assets, total = assets.Count });
    }

    [HttpGet("{id:guid}")]
    public async Task<IActionResult> GetAsset(Guid id)
    {
        AssetReadModel? asset = await readDb.Assets.FindAsync(id);
        return asset == null ? this.NotFound() : this.Ok(asset);
    }

    [HttpPost]
    public async Task<IActionResult> RegisterAsset([FromBody] RegisterAssetRequest request)
    {
        string assetNumber = $"AST-{DateTime.UtcNow:yyyyMMdd}-{Guid.NewGuid().ToString()[..8].ToUpper()}";
        ...
        await eventStore.SaveAggregateAsync(asset);
        return this.CreatedAtAction(nameof(this.GetAsset), new { id = asset.Id }, new { id = asset.Id, assetNumber });
    }

    [HttpPost("{id:guid}/activate")]
    public async Task<IActionResult> Activate(Guid id)
    {
        Asset? asset = await eventStore.LoadAggregateAsync<Asset>(id);
        if (asset == null) return this.NotFound();
    ...
    public async Task<IActionResult> Transfer(Guid id, [FromBody] TransferAssetRequest request)
    {
        ...
        await eventStore.SaveAggregateAsync(asset);
        return this.Ok(new { id, locationId = request.ToLocationId });
    }
    ...
    public async Task<IActionResult> Dispose(Guid id, [FromBody] DisposeAssetRequest request)
    {
        ...
        await eventStore.SaveAggregateAsync(asset);
        return this.Ok(new { id, status = "Disposed" });
    }

    [HttpGet("statistics")]
    public async Task<IActionResult> GetStatistics()
    {
        List<AssetReadModel> assets = await readDb.Assets.ToListAsync();
...
public class MaintenanceController(AssetsReadDbContext readDb) : ControllerBase
{
    [HttpGet]
    ...
    {
        IQueryable<MaintenanceReadModel> query = readDb.MaintenanceRecords.AsQueryable();
        ...
        List<MaintenanceReadModel> records = await query.OrderByDescending(m => m.MaintenanceDate).ToListAsync();
        return this.Ok(new { items = records, total = records.Count, totalCost = records.Sum(r => r.Cost) });
    }
    ...
    public async Task<IActionResult> GetMaintenanceSchedule()
    {
        ...
        List<AssetReadModel> activeAssets = await readDb.Assets.Where(a => a.Status == "Active").ToListAsync();
...
public class DepreciationController(AssetsReadDbContext readDb) : ControllerBase
{
    [HttpGet]
    ...
    {
        IQueryable<DepreciationReadModel> query = readDb.DepreciationRecords.AsQueryable();
        ...
        List<DepreciationReadModel> records = await query.OrderByDescending(d => d.Year).ThenByDescending(d => d.Month).ToListAsync();
        return this.Ok(new { items = records, total = records.Count, totalDepreciation = records.Sum(r => r.Amount) });
    }

    [HttpGet("summary/{year:int}")]
    public async Task<IActionResult> GetYearlySummary(int year)
    {
        List<DepreciationReadModel> records = await readDb.DepreciationRecords.Where(d => d.Year == year).ToListAsync();
    ...
    [HttpPost("run-batch")]
    public Task<IActionResult> RunBatchDepreciation([FromBody] BatchDepreciationRequest request)
    {
        ...
        return Task.FromResult<IActionResult>(this.Ok(new { 
